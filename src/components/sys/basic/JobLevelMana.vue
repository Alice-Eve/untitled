    ...
    <div style="padding-top: 10px">
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
                size="mini" @click="showEdtView(scope.row)">
            编辑</el-button>
         <el-button
             size="mini"
             type="danger" @click="deleteHandle(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <div>
        <table>
          ...
          <tr>
            ...
            <td><el-select v-model="updateJl.titleLevel" placeholder="职称等级" size="small" style="margin-left: 8px;margin-right: 8px">
              <el-option
                  v-for="item in titleLevels"
                  :key="item"
                  :label="item"
...
export default {
  ...
  methods: {
    ...
    doUpdate(){
      this.putRequest("/system/basic/job/",this.updateJl).then(resp=>{
        if (resp) {
          this.initJls();
    ...
    deleteHandle(data) {
      this.$confirm('此操作将永久删除'+data.name+'职称, 是否继续?', '提示', {
        ...
        this.deleteRequest("/system/basic/job/?ids=" + data.id).then(resp => {
          if (resp) {
            this.initJls();
          }
        })
